...
static void draw_particles(GLFWwindow* window, double t, float dt)
{
    ...
    // the matrix, which represents the cube1Rotation.
    glGetFloatv(GL_MODELVIEW_MATRIX, mat);

    // 1) & 2) We do it in one swift step:
    // Although not obvious, the following six lines represent two matrix/
    // vector multiplications. The matrix is the inverse 3x3 cube1Rotation
    ...
    // the opposite corners (regardless of cube1Rotation).
